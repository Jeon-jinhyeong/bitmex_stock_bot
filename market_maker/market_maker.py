...
class OrderManager:
    ...
    def get_price_offset(self, index):
        ...
        if index == 1:
            ...
            return math.toNearest(start_position * 1 ** 1, self.instrument['tickSize'])

        else:

            return math.toNearest(start_position * (1 + settings.INTERVAL) ** (index - 1) ,  self.instrument['tickSize'])
        print(self.instrument['tickSize'])

    def get_price_offset_2(self, index):
        """Given an index (1, -1, 2, -2, etc.) return the price for that side of the book.
           Negative is a buy, positive is a sell."""
        position = self.exchange.get_position()
        # Maintain existing spreads for max profit
        if settings.MAINTAIN_SPREADS:
            start_position = float(settings.BUY[(abs(index) - 1)])
            #start_position = float(position['avgCostPrice'])
            # First positions (index 1, -1) should start right at start_position, others should branch from there
            #index = index + 1 if index < 0 else index - 1
        else:
            # Offset mode: ticker comes from a reference exchange and we define an offset.
        ...
        return math.toNearest(start_position * (1 + settings.BUY_INTERVAL) ** -1, self.instrument['tickSize'])

    def get_price_offset_divide_up(self, index):
        """Given an index (1, -1, 2, -2, etc.) return the price for that side of the book.
           Negative is a buy, positive is a sell."""
        position = self.exchange.get_position()
        # Maintain existing spreads for max profit
    ...
    def get_price_offset_divide_down(self, index):
        """Given an index (1, -1, 2, -2, etc.) return the price for that side of the book.
           Negative is a buy, positive is a sell."""
        position = self.exchange.get_position()
        # Maintain existing spreads for max profit
        ...
        return math.toNearest(start_position * (1 + settings.DIVIDE_INTERVAL) ** -1, self.instrument['tickSize'])
    ...
    def place_orders(self):
        ...
        return self.converge_orders(sell_orders)

    def place_orders_3(self, index):
        """Create order items for use in convergence."""
        ...
        for i in range(1,3):
            if not self.long_position_limit_exceeded():
                    ...
                    buy_orders_2.append(self.prepare_order_3(-i, index))

                    print("up : ", buy_orders_2[i-1])

        return self.converge_orders_2(buy_orders_2)

    def place_orders_2(self):
        """Create order items for use in convergence."""
        ...
        # a new order is created in the inside. If we did it inside-out, all orders would be amended
        # down and a new order would be created at the outside.
        for i in reversed(range((1 + settings.BUY_COUNT), settings.COUNT + 1)): 
            if not self.long_position_limit_exceeded():
                if settings.B[(i - 1)] > settings.DIVIDE_COUNT:

                    self.place_orders_3((i - 1))
    ...
    def prepare_order(self, index):
        ...
        if settings.RANDOM_ORDER_SIZE is True:
            ...
            quantity = settings.A[(abs(index) - 1)]

        price = self.get_price_offset(index)

        return {'price': price, 'orderQty': quantity, 'side': "Buy" if index < 0 else "Sell"}

    def prepare_order_2(self, index):
        """Create an order object."""
        ...
        if settings.RANDOM_ORDER_SIZE is True:
            ...
            #quantity = settings.ORDER_START_SIZE + ((abs(index) - 1) * settings.ORDER_STEP_SIZE)
            quantity = settings.B[(abs(index) - 1)]

        price = self.get_price_offset_2(index)

        return {'price': price, 'orderQty': quantity, 'side': "Buy" if index < 0 else "Sell"}

    def prepare_order_3(self, index, index_2):
        """Create an order object."""
        ...
        if settings.RANDOM_ORDER_SIZE is True:
            ...
            if settings.Divide != 0:
          
                if settings.B[(index_2)] > 9:

                    if abs(index) == 1:

                        quantity = math_2.ceil((settings.B[(index_2)] * settings.Divide) / 100)

                        price = self.get_price_offset_divide_up(index)
                        ...
                        quantity = settings.B[(index_2)] - (math_2.ceil((settings.B[(index_2)] * settings.Divide) / 100))
                            
                        price = self.get_price_offset_divide_down(index)

                        print("down")

                        return {'price': price, 'orderQty': quantity, 'side': "Buy"}

    def converge_orders(self, sell_orders):
    ...
    def converge_orders_2(self, buy_orders):
        """Converge the orders we currently have in the book with what we want to be in the book.
           This involves amending any open orders and creating new ones if any have filled completely.
           We start from the closest orders outward."""
    ...
    def send_email(self):
        ...
        server = smtplib.SMTP('mail.xrosgen.com', 587)

        server.ehlo()
        server.login(settings.email_id, settings.email_pw)

        msg = MIMEText("Bitmex Auto server Down")
        msg['Subject'] = '서버다운'
        msg['From'] = 'bitmex_auto'
        msg['To'] = settings.email_id
        server.sendmail(settings.email_id, settings.email_id, msg.as_string())
        server.close()
    ...
    def exit(self):
        ...
        if settings.email == "local":
            ...
            os.system("echo '서버가 다운됬습니다.' | mail -r bitmex_auto -s '서버 다운' " + str(settings.email_id))
    ...
    def run_loop(self):
        while True:
            ...
            if settings.BUY_STATUS > 0:
                self.place_orders_2()

                settings.BUY_STATUS = 0
