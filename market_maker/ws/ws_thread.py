...
class BitMEXWebsocket():
    ...
    def send_email(self, msg_1, msg_2, order_type, market):
        ...
        if order_type == 'Sell':
            ...
            msg = MIMEText(market + " - Market Price: " + str(msg_1) + " - Order Price: " + str(msg_2) + " - Sell order success")
            msg['Subject'] = '매도주문체결'
            ...
            msg = MIMEText(market + " - Market Price: " + str(float(msg_1)) + " - Order Price: " + str(msg_2) + " - Buy order success")
            msg['Subject'] = '청산주문체결'
            msg['From'] = 'bitmex_auto'
            msg['To'] = settings.email_id
            server.sendmail(settings.email_id, settings.email_id, msg.as_string())
                            ...
                            if 'cumQty' in updateData and not is_canceled:
                                ...
                                if contExecuted > 0:
                                    ...
                                    if settings.email == 'local':
                                        if item['side'] == 'Sell':
                                            os.system("echo " + "'매도주문체결 - " + str(item['symbol']) + " - " + str(item['price']) + " - " + str(contExecuted) + "' | mail -r bitmex_auto -s '매도체결' " + str(settings.email_id))

                                        else:

                                            os.system("echo " + "'청산주문체결 - " + str(item['symbol']) + " - " + str(item['price']) + " - " + str(contExecuted) + "' | mail -r bitmex_auto -s '청산체결' " + str(settings.email_id))

                                    else:
                                        #Send Email
                                        self.send_email(item['price'], contExecuted, item['side'], item['symbol'])

                                    #Buy order
                                    if item['side'] == "Sell":

                                        #if (settings.COUNT % 5) == 0:
                                        #    settings.PAIR_COUNT += 4

                                        settings.BUY.append((float(item['price'])))
                                        settings.B.append(contExecuted)

                                        settings.COUNT += 1
                                        settings.BUY_STATUS = 1
                                    #sys.run()

                                    #price = market_maker.OrderManager.get_price_offset(-1)

                                    #market_maker.OrderManager.create_bulk_orders({'price': price, 'orderQty': contExecuted, 'side': "Buy"})
